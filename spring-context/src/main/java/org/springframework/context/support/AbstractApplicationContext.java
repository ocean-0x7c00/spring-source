...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		/**
		 * 1.为什么要加锁?
		 * 避免多线程同时刷新 Spring 上下文（同时刷新会有什么问题？）
		 *
		 * 2.为什么没有在方法前加锁，而使用了对象锁startUpShutdownMonitor？
		 * 好处
		 * (1)refresh()方法和 close()方法都使用了 startUpShutdownMonitor 对象锁加锁，
		 * 这就保证了在调用 refresh()方法的时候无法调用 close()方法，反之依然，这样就避免了冲突。
		 *
